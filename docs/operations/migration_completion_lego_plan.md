...
**Status (2026-08-02):** In progress under an owner-accepted capacity exception. Candidate SLO/RTO/RPO targets, current provider limits and usage, initial headroom calculations, a bounded read-only probe, and its safety tests are complete. Supabase Free Plan egress is 5.346/5 GB (107%); the owner accepted continued use during the fair-use grace period and directed development and smoke testing to proceed. The possible HTTP 402 restriction after 2026-08-19 remains monitored. Render is at 11.52/750 instance hours (1.536%) and 153 MB/5 GB bandwidth (about 3%). Four warmed local windows passed functional checks, and fresh owner-assisted scheduler health and 28-day sync rollups returned HTTP 200. Exact Render cold-start timing and the reliability-policy decision remain explicitly deferred. Authenticated calendar latency and canary evidence also remain open. Traffic movement and direct Worker data access still require their separate security and promotion gates.
...
**Status (2026-08-02):** In progress. The unauthenticated parity harness covers direct/proxied health, public schema health, static assets, Worker edge-health ownership, Worker-native ownership, proxy parity, and invalid-ticket WebSocket rejection. A direct live run against Render and Cloudflare passed all 18 checks with zero failures; evidence is `artifacts/hosted-smoke-unauthenticated-20260802.json`. The owner designated an existing test account for authenticated smoke use, so no new account is required. A local interactive authenticated run then passed Render create, Cloudflare read/update, Render read-back, and malformed-upload rejection on both targets. Cleanup deleted the temporary event successfully. The run failed closed when deployed Render returned 404 for `/ws/ticket`; both live OpenAPI inventories confirm that the uncommitted WebSocket ticket route is not deployed. Evidence is `artifacts/hosted-smoke-authenticated-20260802.json`. The authenticated harness now also verifies an event-scoped note round trip and cleanup, read-only task/account access, scheduler ownership, TV version/state, and representative assets on both targets. Provider retry/sync, OAuth reconnect, TV pairing, and task creation remain excluded because they can create external effects or persistent residue. Eight focused safety/workflow tests pass. A separate manual-only release workflow verifies the selected commit, deploys the isolated canary, requires both smoke suites, and permits default-off root-Worker promotion only through a protected environment; two structural gate tests pass. GitHub environments and secrets are not recorded as provisioned, no release workflow has run, and a corrected authenticated live pass remains open.
...
4. **Copilot (in progress):** `deployment/authenticated_smoke.py` implements authenticated cross-target event/note verification, read-only task/account/scheduler/TV/asset checks, malformed-upload rejection, and Render/Cloudflare WebSocket echo. Event deletion must also prove the event-scoped note disappeared. The earlier live HTTP/data checks passed, but deployed Render lacks `/ws/ticket`; rerun the expanded matrix after that already-tested local route is deployed.
...
8. **Automatic (in progress):** The authenticated harness exited nonzero when the deployed WebSocket ticket route was missing while still completing cleanup. The release workflow enforces both canary smoke jobs before promotion; live enforcement remains unproven until the first post-development release.
9. **Gate (in progress):** The 2026-08-02 live run proves a missing required route fails the smoke gate and preserves cleanup. A corrected authenticated pass after deployment remains required.
10. **Stop/rollback:** Invoke the last known-good Worker rollback and Render rollback procedures when post-deploy smoke fails.

**Pass evidence:** The direct credential-free hosted run passed 18/18. The authenticated run captured a required-route failure with successful cleanup. A corrected authenticated pass and GitHub-hosted run links remain required.
...
**Status (2026-08-02):** Repository preparation is in progress. Wrangler now defines an isolated `canary` environment on `workers.dev`, CI dry-runs both root and canary bundles, and production configuration rejects callback URLs that do not exactly match `BASE_URL` plus the provider callback path. Worker origin failures use bounded structured JSON that excludes query strings and headers. Focused callback-drift, release-gate, and Worker tests pass. Choosing and attaching an operator hostname, provider registration, secret configuration, deployment, functional verification, and the seven-day observation gate remain owner/live actions.

1. **Owner:** Choose an operator-only hostname in the Cloudflare-managed zone, for example `canary.<your-domain>`.
2. **Copilot (in progress):** The separate `canary` Worker environment is configured and locally testable on `workers.dev`. Attach a route only after the owner chooses the operator hostname; production bindings remain untouched.
...
**Status (2026-08-02):** In progress. `docs/architecture/async_job_migration_decision.md` inventories all three APScheduler jobs, ownership, triggers, state, database/provider side effects, retries, overlap controls, and migration readiness. Focused tests now prove idempotent diagnostic pruning and daily-rollup update behavior. The repository recommendation is `defer`: event sync lacks durable operation IDs, replay state, and distributed ownership; the rollup depends on process-local counters; diagnostic pruning is the only plausible Cron proof after an exclusive-owner control. Production volume capture, operation ledger, replay suite, cost comparison, and owner decision remain open.
...
2. **Copilot:** Assign an immutable operation ID and idempotency key to every candidate job.
3. **Copilot:** Add a durable operation ledger in authoritative PostgreSQL with pending, running, succeeded, failed, and dead-letter states.
4. **Copilot:** Add unit/integration tests proving duplicate delivery writes once, retries resume safely, and poison messages stop retrying.
