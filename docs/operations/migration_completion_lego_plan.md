...
**Status (2026-08-05):** In progress with corrected authenticated validation complete. The unauthenticated parity harness covers direct/proxied health, public schema health, static assets, Worker edge-health ownership, Worker-native ownership, proxy parity, and invalid-ticket WebSocket rejection. A direct live run against Render and Cloudflare passed all 18 checks with zero failures; evidence is `artifacts/hosted-smoke-unauthenticated-20260802.json`. The owner designated an existing test account for authenticated smoke use, so no new account is required. The earlier authenticated run failed closed when deployed Render returned 404 for `/ws/ticket` and cleanup still succeeded; evidence is `artifacts/hosted-smoke-authenticated-20260802.json`. After `/ws/ticket` deployment and Worker grant fixes, authenticated smoke now passes end-to-end for login, reversible event/note flow, read-only tasks/accounts/scheduler/TV checks, malformed-upload rejection, and WebSocket echo. Provider retry/sync, OAuth reconnect, TV pairing, and task creation remain excluded because they can create external effects or persistent residue. Eight focused safety/workflow tests pass. A separate manual-only release workflow verifies the selected commit, deploys the isolated canary, requires both smoke suites, and permits default-off root-Worker promotion only through a protected environment; two structural gate tests pass.
...
**Status (2026-08-05):** In progress. `docs/architecture/async_job_migration_decision.md` inventories all three APScheduler jobs, ownership, triggers, state, database/provider side effects, retries, overlap controls, and migration readiness. Focused tests prove idempotent diagnostic pruning and daily-rollup update behavior. A durable operation-ledger foundation now exists for all three current APScheduler jobs (`event_sync_job`, `tv_diag_prune_job`, `sync_efficiency_rollup_job`): deterministic operation keys, attempt counts, and terminal status transitions (`succeeded`, `retry_pending`, `dead_letter`) are persisted in `sync_operation_ledger`. Scheduler execution now includes a minimal dead-letter guard that skips already-poisoned operation keys and a default-safe owner gate (`SYNC_SCHEDULER_OWNER=render`) to prevent dual execution during migration phases. Production volume capture, replay suite expansion, cost comparison, and owner decision remain open.
