...
The bounded calendar, legacy event, date-filtered note, task, date-sticky, tag-color, current-user, and TV-version reads are integrated behind route ownership controls with `proxy`, `shadow`, `canary`, and `native` modes. Invalid or missing mode configuration resolves to `proxy`. Production reads are native; event, date-sticky, and tag-color writes are native; standalone note/task writes remain proxy-owned. Canary reads remain in shadow mode and canary writes remain proxy-owned. Non-proxy database execution uses strict RS256 authentication and the cache-disabled `HYPERDRIVE_RLS_NO_CACHE` binding. Supabase remains authoritative, and secret values remain outside the repository.
