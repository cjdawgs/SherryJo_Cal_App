...
1. Apply Alembic revisions through `an994h44aaa88` while Render remains the scheduler owner. These add scheduler claims, native authentication/account access, atomic TV pairing, persistent TV state and diagnostics, one-time WebSocket tickets, app-admin RLS policies, and Worker write-receipt grants without changing runtime ownership.
